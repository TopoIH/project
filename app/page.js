...
export default function EmailExtractor() {
    ...
    const [cleanMode, setCleanMode] = useState(false); // Track the clean option
    ...
    // Password Protection Screen (Slate Gray & Emerald Green)
    if (!isLoggedIn) {
        return (
                ...
                <div style={{ backgroundColor: '#475569', borderRadius: '8px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.2)', padding: '30px', width: '100%', maxWidth: '480px' }}>
                    ...
                    <div style={{ marginBottom: '15px' }}>
                        <input 
                            ...
                            style={{ ...inputStyle, padding: '12px', fontSize: '15px' }} 
                        />
                    </div>
                    <button 
                        onClick={(e) => {
                        ...
                        style={{ width: '100%', padding: '12px', backgroundColor: '#10b981', color: 'white', border: 'none', borderRadius: '6px', fontWeight: '700', fontSize: '16px', cursor: 'pointer' }}
                    >Enter</button>
                </div>
            </div>
        );
        ...
        <div style={{ minHeight: '100vh', backgroundColor: '#f1f5f9', padding: '40px', fontFamily: 'system-ui, sans-serif' }}>
            <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
                ...
                <form action={async (formData) => {
                    setLoading(true);
                    // Pass the cleanMode state to the Server Action
                    ...
                    <div style={{ flex: 2 }}>
                        ...
                        <input name="email" type="email" required placeholder="example@gmail.com" style={inputStyle} />
                    </div>

                    <div style={{ flex: 1.5 }}>
                        <label style={labelStyle}>Label/Folder:</label>
                        <input name="folder" type="text" defaultValue="INBOX" placeholder="e.g. [Gmail]/Spam" style={inputStyle} />
                    </div>
                    ...
                    <div style={{ width: '60px' }}>
                        ...
                        <input name="start" type="number" defaultValue="1" min="1" style={inputStyle} />
                    </div>

                    <div style={{ width: '60px' }}>
                        <label style={labelStyle}>Get:</label>
                        <input name="count" type="number" defaultValue="5" min="1" style={inputStyle} />
                    </div>

                    {/* NEW: Clean Headers Option */}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                    {emails.length > 0 && (
                        <button 
                            onClick={() => {
                                ...
                                alert('All sources copied with __SEP__');
                            }}
                            style={{ alignSelf: 'flex-end', padding: '10px 20px', backgroundColor: '#10b981', color: 'white', border: 'none', borderRadius: '6px', fontWeight: 'bold', cursor: 'pointer' }}
                        >
                    ...
                    {emails.map((msg) => (
                        <div key={msg.id} style={{ backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 4px 6px rgba(0,0,0,0.05)', display: 'flex', height: '400px', overflow: 'hidden', border: '1px solid #e2e8f0' }}>
                            {/* Info Side */}
                            <div style={{ width: '300px', padding: '20px', borderRight: '1px solid #f1f5f9', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                                <div>
                                    <p style={labelStyle}>Sender</p>
                                    <p style={{ fontSize: '13px', fontWeight: '600', marginBottom: '15px', wordBreak: 'break-all' }}>{msg.from}</p>
                                    <p style={labelStyle}>Subject</p>
                                    <p style={{ fontSize: '12px', color: '#475569', lineHeight: '1.4' }}>{msg.subject}</p>
                                </div>
                                <button 
                                    onClick={() => { navigator.clipboard.writeText(msg.source); alert('Copied!'); }}
                                    style={{ width: '100%', padding: '8px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', border: '1px solid #e2e8f0', borderRadius: '6px', background: '#f8fafc' }}
                                >
                                    Copy Source
                                </button>
                            </div>
